...
void Strassen_algorithm(vector<vector<int>> &A, vector<vector<int>> &B, vector<vector<int>> &C, int size)
{
    ...
    else
    {
        ...
        Strassen_algorithm (aResult, bResult, p1, new_size);
        add(a21, a22, aResult, new_size);
        Strassen_algorithm (aResult, b11, p2, new_size);
        sub(b12, b22, bResult, new_size);
        Strassen_algorithm (a11, bResult, p3, new_size);
        sub(b21, b11, bResult, new_size);
        Strassen_algorithm(a22, bResult, p4, new_size);
        add (a11, a12, aResult, new_size);
        Strassen_algorithm(aResult, b22, p5, new_size);
        sub(a21, a11, aResult, new_size);
        add(b11, b12, bResult, new_size);
        Strassen_algorithm(aResult, bResult, p6, new_size);
        sub(a12, a22, aResult, new_size);
        add(b21, b22, bResult, new_size);
        Strassen_algorithm(aResult, bResult, p7, new_size);
...
int main()
{
    vector<vector<int >> a = {{2, 4}, {3, 5}};
    cout<<"\nFirst matrix input:"<<endl;
    display(a,2,2);
    vector<vector<int>> b = {{2, 4}, {5, 3}};
    cout<<"\nSecond matrix input: "<<endl;
    display (b,2,2);
